...
import com.module.util.{Files, StopWords}
...
object InvertedIndexSortByWordsAndCountsWithStopWordsFiltering {
  def main(args: Array[String]): Unit = {
    ...
    val sc = new SparkContext("local[*]", "Invert Index Sort By Words And Counts With Sto Words Filter")
    ...
    try {
      ...
      input
        ...
        .filter {
          case ((word, _), _) ⇒
            !stopWords.value.contains(word) && numbersRE.findFirstIn(word).isEmpty
        }
        .reduceByKey {
